...
async function processCustomItemsPipeline(requestId, profileId, checklistTable, pendingTable) {
  try {
    ...
    const [{ data: checklist }, { data: alreadyPending }] = await Promise.all([
      supabase.from(checklistTable).select('id, item_name').eq('is_active', true),
      supabase.from(pendingTable).select('item_name').eq('status', 'pending'),
    ])

    const checklistMap = new Map(
      (checklist ?? []).map(i => [i.item_name.toLowerCase().trim(), i.item_name])
    )
    const pendingSet = new Set(
      (alreadyPending ?? []).map(i => i.item_name.toLowerCase().trim())
    )
    ...
    for (const item of reqItems) {
      ...
      if (checklistMap.has(key)) {
        await supabase
          .from('request_items')
          .update({ item_name: checklistMap.get(key), is_custom: false })
          .eq('id', item.id)
    ...
    if (toPending.length) {
      await supabase.from(pendingTable).insert(toPending)
    }
  } catch {
    // Silent fail — request already saved, don't block navigation
  }
}
